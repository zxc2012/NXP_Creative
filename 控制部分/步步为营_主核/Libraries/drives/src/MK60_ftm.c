 ...
 * @Target core		MK60DN and MK60F15
...
void FTM_port_mux(FTMn ftmn, FTM_CHn ch)
{
    ...
    case ftm2:
        ...
        break;
     #if (defined(MK66F18) || defined(MK60F15)) 
     case ftm3:
        SIM->SCGC3 |= SIM_SCGC3_FTM3_MASK;       //使能FTM3时钟
        switch(ch)
        {
        case ftm_ch0:
            if(FTM3_CH0_PIN == E5)
...
void ftm_pwm_init(FTMn ftmn, FTM_CHn ch, uint32 freq, uint32 duty)
{
    ...
    // 以 CPWMS = 0 ，即单边捕捉脉冲为例
    clk_hz = (bus_clk_mhz * 1000 * 1000) ;   // bus频率10^8
...
void motor_set(int32 wheel1,int32 wheel2)
{
    if(wheel1>0)
    {
    wheel1 += DEAD_ZONE_L;
    wheel1 = XIANFU(wheel1,0,950);
    ftm_pwm_duty(ftm0, MOTOR_L1_CH, 475+wheel1/2);
    ftm_pwm_duty(ftm0, MOTOR_L2_CH, 475-wheel1/2);
    }
    else if(wheel1<0)
    {
    wheel1 -= DEAD_ZONE_L;
    ...
    {
     ftm_pwm_duty(ftm0, MOTOR_L1_CH, 0);
     ftm_pwm_duty(ftm0, MOTOR_L2_CH, 0);
    }

    if(wheel2>0)
    {
    wheel2 += DEAD_ZONE_R;
    wheel2 = XIANFU(wheel2,0,950);
    ftm_pwm_duty(ftm0, MOTOR_R1_CH, 475+wheel2/2);
    ftm_pwm_duty(ftm0, MOTOR_R2_CH, 475-wheel2/2);
    }
    else if(wheel2<0)
    {
    wheel2 -= DEAD_ZONE_R;
    ...
    {
     ftm_pwm_duty(ftm0, MOTOR_R1_CH, 0);
     ftm_pwm_duty(ftm0, MOTOR_R2_CH, 0);
    }
