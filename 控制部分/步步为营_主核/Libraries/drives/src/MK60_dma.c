...
//-------------------------------------------------------------------------------------------------------------------
void dma_portx2buff_init(DMA_CHn CHn, void *SADDR, void *DADDR, PTX_n ptx_n, DMA_BYTEn byten, uint32 count, uint32 cfg)
{
    ...
    DMAMUX0->CHCFG[CHn] = (0
                            ...
                            | DMAMUX_CHCFG_SOURCE( PTX(ptx_n) + DMA_PORTA)  // 通道触发传输源:    
                          );
    

    //配置触发源（默认是 上升沿触发）
    port_init(ptx_n, ALT1 | DMA_RISING);
